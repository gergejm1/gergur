...
public sealed class AgentDispatchTests
{
    ...
    public void ClosingATabWithoutSayingWhichIsRefused(string? args)
    {
        ...
        Assert.Equal("index", Missing("gergur_close_tab", args));
        Assert.Equal("index", Missing("gergur_activate_tab", args));
    }
    ...
    public void EveryRequiredArgumentIsDeclaredInTheToolSchema()
    {
        ...
        foreach (var tool in doc.RootElement.EnumerateArray())
        {
            ...
            var declared = tool.GetProperty("inputSchema").GetProperty("required")
            ...
            Assert.Equal(declared.Order(), AgentServer.RequiredArgsForTool(name).Order());
        }
    }
}
